...
function ChartTooltipContent(props) {
    ...
    // Fix for recharts payload/label type errors
    ...
    const tooltipLabel = React.useMemo(() => {
        ...
        const value = !labelKey && typeof typedLabel === "string"
            ? config[typedLabel]?.label || typedLabel
            : itemConfig?.label;
        if (labelFormatter) {
    ...
    return (_jsxs("div", { className: cn("border-border/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl", className), children: [!nestLabel ? tooltipLabel : null, _jsx("div", { className: "grid gap-1.5", children: typedPayload
